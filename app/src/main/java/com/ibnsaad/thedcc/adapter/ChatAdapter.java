...
import com.ibnsaad.thedcc.utils.TimeAgo;
...
public class ChatAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public ChatAdapter(List<Message> items, Context ctx) {
        this.items = items;
        this.ctx = ctx;
    }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        if (holder instanceof ItemViewHolder) {
            ...
            timeAgo.locale(ctx);
            String result = timeAgo.parcNaderDate(m.getMessageSent());
    ...
    public int getItemViewType(int position) {
        return this.items.get(position).getSenderId().toString().equals(SharedHelper.getKey(ctx, Enums.ID.name())) ? CHAT_ME : CHAT_YOU;
    }
